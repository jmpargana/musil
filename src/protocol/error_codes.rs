...
mod tests {
    ...
    #[test]
    fn none_is_zero() {
        assert_eq!(i16::from(ErrorCode::None), 0);
    }

    #[test]
    fn unknown_server_error_is_minus_one() {
        assert_eq!(i16::from(ErrorCode::UnknownServerError), -1);
    }

    #[test]
    fn last_variant_roundtrips() {
        let code = ErrorCode::ShareSessionLimitReached;
        let n = i16::from(code);
        assert_eq!(n, 133);
        assert_eq!(
            ErrorCode::try_from(n),
            Ok(ErrorCode::ShareSessionLimitReached)
        );
    }
    ...
    fn all_variants_roundtrip() {
        for n in -1i16..=133 {
            let code = ErrorCode::try_from(n).expect(&format!("{n} should be valid"));
            assert_eq!(i16::from(code), n);
        }
    }

    #[test]
    fn unknown_value_returns_err() {
        assert_eq!(ErrorCode::try_from(200i16), Err(200));
        assert_eq!(ErrorCode::try_from(-2i16), Err(-2));
    }
}
